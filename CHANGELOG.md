...
### 修 4 个 Bug
1. **LLM provider 选错崩溃**：环境有 `ANTHROPIC_API_KEY` 但没装 `anthropic` SDK 时，仍会选 claude 然后 ImportError 500。修：`_resolve_provider` 增加 SDK 存在性检查（`importlib.util.find_spec`）。
2. **股票名字全变成"沪深300"**：AkShare 返回的 `index_stock_cons_csindex` 有"指数名称"和"成分券名称"两列，我原来的 name_col 匹配匹到了前者。修：优先匹配"成分券名称"。
3. **单只网络失败 crash**：AkShare 拉不到数据时抛异常直接 500。修：`analysis/scorer.py` 每一维度独立 try/except，任何一维失败降级为中性 50，不影响其他。
4. **os 未导入**：新加的 settings 端点用了 `os.environ.get` 但没 import。

### 重设计 UI（面向普通用户）
- **首页 = 智能选股器**：5 种"选股风格"卡片（均衡/动量/价值/成长/红利）+ 4 步向导（选池→池大小→分数下限→返回数量）
- **每个选项都有 tooltip**：解释是什么、该选什么
- **配色和层级**：分数条 + 颜色（>=80 红/看好，>=65 橙，<50 绿/回避）
- **搜索框**：输代码或名称，实时搜索建议下拉
- **点股票卡片直接跳详情页**
- **新增"使用指南"tab**：给新手看的 6 个常见问题
- **所有配置集中到"设置"页**：LLM key（脱敏显示）、通知渠道、策略参数（止损止盈仓位天数）
- **敏感 key 脱敏保护**：显示 `abcd***wxyz`，保存时留空视为不改（避免误覆盖）

### 新增后端 API
- `POST /api/screen` — 一站式股票筛选（首页核心）
- `GET /api/screen/presets` — 5 种预设权重方案
- `GET /api/market/search` — 按代码/名称搜索
- `GET /api/settings` — 读所有设置（敏感值脱敏）
- `POST /api/settings` — 保存 .env 和 configs/strategy.yaml
...
| 价值 | 15% | 55% | 10% | 20% | 中长线 |
| 成长 | 25% | 40% | 20% | 15% | 找爆发股 |
| 红利 | 10% | 60% | 10% | 20% | 长线拿股息 |

### 测试
- pytest：29/29 全绿（新增 1 个 SDK 缺失时的兜底测试）
